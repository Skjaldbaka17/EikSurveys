...
async function addTimeToAllSurveys(){
    ...
    try{
        ...
        for(var i = 0; i < allThings.length; i++){
            query = `update ${surveysDB} set (questions) values($1) where surveyid = ${allThings[i].id}`
...
async function createAnswersTable(questions, name){
    ...
    try{
        ...
        for(var i = 0; i < questions.length; i++){
            query += ( await onlyLetters(questions[i].question)) + " TEXT" + (questions[i].multipleAnswers ? "[]":"")
